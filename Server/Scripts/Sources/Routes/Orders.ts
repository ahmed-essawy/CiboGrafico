...
import { Order_Rate, objectId, OrderType } from "../Types";
{
    ...
    orders
        ...
        .get("/userMeal/:num", (req, res) => db.Orders.ReadFull(req.params.num, response => res.json(response)))
        ...
        .post("/", (req, res) => {
            if (req.body.owner && req.body.restaurant && req.body.type && Array.isArray(req.body.meals)) {
                ...
                db.lastOrderNum().then(num => {
                    const tempOrder = new Order(db.objectId(), num, req.body.owner, req.body.restaurant, req.body.type, tempAddress, req.body.meals);
        ...
        .put("/", (req, res) => {
            if (req.body._id) {
                db.Orders.Read({ _id: req.body._id }, response => {
                    if (response.success) {
