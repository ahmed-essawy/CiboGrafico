...
import { Order_Rate, objectId, OrderType } from "../Types";
{
    ...
    orders
        ...
        .get("/UserOrder/:id",
            (req, res) => db.Orders.ReadAllByUser({ _id: req.params.id }, response => res.json(response)))
        .get("/SubOrders/:num",
            (req, res) => db.Orders.GetSubOrders(parseInt(req.params.num),
                response => res
                .json(response)))
        .delete("/:id", (req, res) => db.Orders.Delete({ _id: req.params.id }, response => res.json(response)))
        .post("/",
            (req, res) => {
                if (req.body.owner && req.body.restaurant && req.body.type && Array.isArray(req.body.meals)) {
                    let tempAddress = new Address();
                    if (req.body.address) {
                        tempAddress = new Address(req.body.address.street, req.body.address.city, req.body.address
                            .country);
                    }
                    db.lastOrderNum().then(num => {
                        console.log(req.body.restaurant);
                        const tempOrder = new Order(db.objectId(), num, req.body.owner, req.body.restaurant, req.body
                            .type, tempAddress, req.body.meals);
                        db.Orders.Create(tempOrder, response => res.json(response));
                    }).catch(err => { res.status(400).json({ success: false, msg: "Error while get data" }); });
