...
import { Order_Rate, objectId, OrderType } from "../Types";
{
    ...
    orders
        ...
        .get("/SubOrders/:num", (req, res) => db.Orders.GetSubOrders(parseInt(req.params.num), response => res
            .json(response)))
        .delete("/:id", (req, res) => db.Orders.Delete({ _id: req.params.id }, response => res.json(response)))
        .post("/", (req, res) => {
            if (req.body.num && req.body.owner && req.body.restaurant && req.body.type && Array.isArray(req.body
                .meals)) {
                let tempAddress = new Address();
                if (req.body.address)
                    tempAddress = new Address(req.body.address.street, req.body.address.city, req.body.address.country);
                const tempOrder = new Order(db.objectId(), req.body.num, req.body.owner, req.body.restaurant, req.body
