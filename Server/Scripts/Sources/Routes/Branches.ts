...
const valid = new Validator();
{
    ...
    branches
        ...
        .post("/", (req, res) => {
            if (req.body.restaurant && req.body.branch && req.body.branch.manager && req.body.branch.address && valid.isEmail(req.body.branch.email) && valid.isArray(req.body.branch.phones) && req.body.branch.username && req.body.branch.password) {
                const tempManager = new Manager(req.body.branch.manager.firstName, req.body.branch.manager.lastName, req.body.branch.manager.phones);
                const tempAddress = new BranchAddress(req.body.branch.address.area, req.body.branch.address.city, req.body.branch.address.country, req.body.branch.address.street);
                const tempBranch = new Branch(db.objectId(), req.body.branch.name, tempManager, tempAddress, req.body.branch.email, req.body.branch.username, req.body.branch.phones, req.body.branch.maximumGuests, req.body.branch.guestsPerTable);
                db.Branches.Create(tempBranch, req.body.branch.password, { _id: req.bodytempAddressrestaurant }, response => res.json(response));
                ...
                db.Branches.Read({ _id: req.body._id },
                    response => {
                        ...
                        if (req.body.manager) {
                            ...
                            if (valid.isArray(req.body.manager.phones)) tempBranch.manager.phones = req.body.manager.phones;
                            if (req.body.manager.address) {
                                if (req.body.manager.address.area) tempBranch.manager.address.country = req.body.manager.address.area;
