...
module.exports = {
        ...
        Read(object: Id, callback: any) {
            Collection("Orders").findOne(objectId(object._id), (err, row: Order) => {
        ...
        Create(object: Order, callback: any) {
            ...
            Collection("Orders").update({ "num": object.num }, { $setOnInsert: object }, { upsert: true },
                (err, resp) => {
        ...
        RateOrder(object: Order_Rate, callback: any) {
            object._id = objectId(object._id);
            Collection("Orders").updateOne({ _id: object._id }, { $set: { rate: object.rate } },
                (err, row: Order) => {
                    if (row) return callback({ success: true, data: row });
                    else return callback({ success: false });
                });
